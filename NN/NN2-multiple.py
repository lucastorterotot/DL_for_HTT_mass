...
# make Higgs E
df["Higgs_E_gen"] = (df["Higgs_mass_gen"]**2 + (df["Higgs_pt_gen"]*np.cosh(df["Higgs_eta_gen"]))**2)**(.5)

# define target and input variables
Higgs_gen_variable_targets = ["pt", "eta", "E"]
targets = ["Higgs_{}_gen".format(Higgs_gen_variable_target) for Higgs_gen_variable_target in Higgs_gen_variable_targets]
inputs = list(df.keys())
...
arr_y_train = np.r_[df_y_train]
arr_x_valid = np.r_[df_x_valid]
arr_y_valid = np.r_[df_y_valid]
arr_x_test  = np.r_[df_x_test]
arr_y_test  = np.r_[df_y_test]
...
NN_model.add(Dense(100, activation="relu"))
NN_model.add(Dense(100, activation="relu"))
NN_model.add(Dense(len(targets), activation="linear"))
...
for k, target in enumerate(targets):
    ...
    fig, ax = plt.subplots()
    predictions, answers = NN_model.predict(arr_x_test)[:,k], arr_y_test[:,k]

    # Calculate the point density
    from matplotlib.colors import Normalize
    from scipy.interpolate import interpn
    data , x_e, y_e = np.histogram2d( answers, predictions, bins = [30,30], density = True )
    ...
    plt.xlabel("Generated Higgs {} (GeV)".format(target.split("_")[1]))
    plt.ylabel("Predicted Higgs {} (GeV)".format(target.split("_")[1]))
    ...
    #plt.show()
    fig.savefig("predicted_vs_answers_{}.png".format(target))
...
plt.rcParams["figure.figsize"] = [16, 10]
fig, ax = plt.subplots()
_predictions, _answers = NN_model.predict(arr_x_test), arr_y_test

Higgs_pt_gen_idx = targets.index("Higgs_pt_gen")
Higgs_eta_gen_idx = targets.index("Higgs_eta_gen")
Higgs_E_gen_idx = targets.index("Higgs_E_gen")

answers = (_answers[:,Higgs_E_gen_idx]**2-(_answers[:,Higgs_pt_gen_idx]*np.cosh(_answers[:,Higgs_eta_gen_idx]))**2)**(.5)
predictions = (_predictions[:,Higgs_E_gen_idx]**2-(_predictions[:,Higgs_pt_gen_idx]*np.cosh(_predictions[:,Higgs_eta_gen_idx]))**2)**(.5)
